 ...
 */
  async swapTokens(params: types.SwapTokensParams): Promise<ServiceMethodResponseDto<TransactionModel>> {
    try {
      ...
      const transaction = await this.transactionRepo.save({
        ...
        created_at: moment().format(DB_DATE_FORMAT),
        updated_at: moment().format(DB_DATE_FORMAT),
        service_fee: wallet.network === Network.TON ? 0.045 :(amount * 100) / 10000,
        service_fee_usd: wallet.network === Network.TON ? 0.001 :(amount * fromToken.price * 100) / 10000
      });
  ...
  async processReferralCommission(transactionId: string): Promise<boolean> {
    try {
      ...
      // Находим кошелёк по wallet_id, указанному в транзакции
      ...
      // Находим реферальную запись для пользователя, связанного с этим кошельком
      ...
      // Проверяем, был ли пользователь приглашён кем-либо
      ...
      // Обновляем статус транзакции, отмечая её как обработанную
      await this.transactionRepo.update({ id: transactionId }, { is_referral_processed: true });
