    ...
    try {
      ...
      switch (network) {
        ...
        case Network.ETH:
          ...
          // Always set approval for non-native tokens
          if (sellTokenAddress !== nativeTokenAddress) {
            ...
            console.log('Max approval set. Transaction:', approveTx);
            
            // Wait for the approval transaction to be mined
            if ('txId' in approveTx) {
              // It's a TransactionHash type
              await sdk.blockchain.getTransaction(approveTx.txId);
              console.log('Approval transaction mined');
            } else if ('signatureId' in approveTx) {
              // It's a SignatureId type
              console.log('Approval signature created with ID:', approveTx.signatureId);
              // Here you might want to implement a way to wait for this signature to be processed
              // This depends on your specific implementation and requirements
            } else {
              console.log('Unexpected approval response type:', approveTx);
            }
          }
