    ...
    try {
      ...
      switch (network) {
        ...
        case Network.ETH:
          ...
          // Function to get quote with retries and amount adjustment
          const getQuoteWithRetry = async (attemptAmount: bigint, maxRetries = 3): Promise<any> => {
            for (let i = 0; i < maxRetries; i++) {
              try {
                const quoteParams = new URLSearchParams({
                  ...
                  sellAmount: attemptAmount.toString(),
                  takerAddress: fromAddress,
                });

                console.log(`Attempt ${i + 1} - Quote API Request URL:`, `${zeroXApiUrl}/swap/v1/quote?${quoteParams}`);
                ...
                if (!response.ok) {
                  ...
                  console.error(`Attempt ${i + 1} - 0x API Quote Error:`, errorText);
                  
                  if (response.status === 400) {
                    const errorJson = JSON.parse(errorText);
                    if (errorJson.validationErrors?.[0]?.code === 1004) {
                      // Reduce the amount and retry
                      attemptAmount = attemptAmount * BigInt(95) / BigInt(100); // Reduce by 5%
                      console.log(`Reducing amount to ${attemptAmount} and retrying...`);
                      continue;
                    }
                  }
                ...
                console.log(`Attempt ${i + 1} - Quote Data:`, JSON.stringify(quoteData, null, 2));
                return quoteData;
              } catch (error) {
          ...
          const quoteData = await getQuoteWithRetry(sellAmountWei);
