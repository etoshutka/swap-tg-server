    ...
    try {
      ...
      switch (network) {
        ...
        case Network.ETH:
          ...
          if (quoteData.issues?.allowance && sellTokenAddress !== nativeTokenAddress) {
          ...
          // Execute swap
          const swapTx: any = await sdk.erc20.send.approveSignedTransaction({
            contractAddress: sellTokenAddress,
            spender: quoteData.issues.allowance.spender,
            amount: quoteData.sellAmount,
            fromPrivateKey,
          ...
          const transaction = await sdk.blockchain.getTransaction(swapTx.txId);

          
          console.log('Swap Transaction Result:', JSON.stringify(swapTx, null, 2));
